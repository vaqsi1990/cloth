  ...
  try {
    ...
    const orderItem = await prisma.orderItem.findUnique({
      ...
      include: {
        ...
        product: { select: { userId: true, stock: true } },
      },
    })
    ...
    if (orderItem.sellerReportedOutOfStock) {
      return NextResponse.json({
        success: true,
        message: 'უკვე მითითებულია, რომ მარაგში არ გაქვთ',
    ...
    const updated = await prisma.orderItem.update({
      ...
      data: {
        ...
        sellerReportedAt: new Date(),
      },
      select: {
        id: true,
        sellerReportedOutOfStock: true,
        sellerReportedAt: true,
      },
    })

    return NextResponse.json({
      success: true,
      message: 'ადმინისტრატორს გადაეცემა ინფორმაცია, რომ ნივთი მარაგში არ გაქვთ',
      item: updated,
    })
  } catch (error) {
