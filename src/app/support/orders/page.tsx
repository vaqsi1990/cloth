...
const SupportOrdersPage = () => {
  ...
  const handleCapturePaymentHold = async (orderId: number) => {
    if (
      !confirm(
        'დარწმუნებული ხართ, რომ გსურთ გადახდის დადასტურება? BOG-ზე თანხა გადანაწილდება მერჩანტს (კომისია/მიტანა) და გამყიდველს (IBAN-ზე), თუ split ჩართულია.',
      )
    ) {
      return
    }
                      ...
                      {order.paymentHoldStatus === 'CAPTURED' && (
                        <div className="mb-3 sm:mb-4 p-2 sm:p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                          <p className="text-xs sm:text-sm text-emerald-800">
                            BOG-ში გადახდა დადასტურებულია (completed). თანხა მერჩანტ ანგარიშზე შეიძლება 1-2 სამუშაო დღეში გამოჩნდეს.
                          </p>
                        </div>
                      )}
